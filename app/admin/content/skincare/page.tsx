...
import { updateSkincarePage } from "./actions";

export default async function AdminSkincareContentPage() {
  ...
  const concernItems = concerns.items ?? [];
  const skinTypes = content.skinTypes ?? {};
  const skinTypeItems = skinTypes.items ?? [];
      ...
      <div className="mx-auto max-w-5xl">
        <div className="border-b border-stone-200 pb-8">
          ...
          <p className="mt-3 text-sm leading-6 text-stone-500">
            Manage the content displayed on your skincare page.
          </p>
        </div>

        <form
          action={updateSkincarePage}
        ...
        >
          ...
          <Section title="Concerns">
            <div className="grid gap-6 sm:grid-cols-2">
              <Field
                label="Eyebrow"
                name="concernsEyebrow"
                value={concerns.eyebrow}
              />

              <Field
                label="Title Before"
                name="concernsTitleBefore"
                value={concerns.titleBefore}
              />

              <Field
                label="Highlight"
                name="concernsHighlight"
                value={concerns.highlight}
              />
            ...
            {Array.from({ length: 6 }, (_, index) => (
              ...
              >
                ...
                <div className="mt-5 grid gap-6 sm:grid-cols-2">
                  <Field
                    label="Title"
                    name={`concern${index + 1}Title`}
                    value={concernItems[index]?.title}
                  />

                  <Field
                    label="Symbol"
                    name={`concern${index + 1}Symbol`}
                    value={concernItems[index]?.symbol}
                  />

                  <Field
                    label="Description"
                    name={`concern${index + 1}Description`}
                    value={concernItems[index]?.description}
                  />

                  <Field
                    label="Filter"
                    name={`concern${index + 1}Filter`}
                    value={concernItems[index]?.filter}
                  />
                </div>
              </div>
            ))}
          ...
          <Section title="Skin Types">
            <div className="grid gap-6 sm:grid-cols-2">
              <Field
                label="Eyebrow"
                name="skinTypesEyebrow"
                value={skinTypes.eyebrow}
              />

              <Field
                label="Title Before"
                name="skinTypesTitleBefore"
                value={skinTypes.titleBefore}
              />

              <Field
                label="Highlight"
                name="skinTypesHighlight"
                value={skinTypes.highlight}
              />
            </div>
            ...
            {Array.from({ length: 4 }, (_, index) => (
              ...
              >
                ...
                <div className="mt-5 grid gap-6 sm:grid-cols-2">
                  <Field
                    label="Title"
                    name={`skinType${index + 1}Title`}
                    value={skinTypeItems[index]?.title}
                  />

                  <Field
                    label="Symbol"
                    name={`skinType${index + 1}Symbol`}
                    value={skinTypeItems[index]?.symbol}
                  />

                  <Field
                    label="Description"
                    name={`skinType${index + 1}Description`}
                    value={skinTypeItems[index]?.description}
                  />

                  <Field
                    label="Filter"
                    name={`skinType${index + 1}Filter`}
                    value={skinTypeItems[index]?.filter}
                  />
                </div>
              </div>
            ))}
          ...
          <Section title="Routine">
            <div className="grid gap-6 sm:grid-cols-2">
              ...
              <Field
                ...
                value={routine.buttonText}
              />

              <Field
                label="Button Link"
                name="routineButtonLink"
                value={routine.buttonLink}
              />
            </div>
            ...
            {Array.from({ length: 4 }, (_, index) => (
              ...
              >
                <h3 className="font-serif text-2xl">
                  Routine Step {index + 1}
                </h3>

                <div className="mt-5 grid gap-6 sm:grid-cols-2">
                  <Field
                    label="Number"
                    name={`routine${index + 1}Number`}
                    value={routineSteps[index]?.number}
                  />

                  <Field
                    label="Title"
                    name={`routine${index + 1}Title`}
                    value={routineSteps[index]?.title}
                  />

                  <div className="sm:col-span-2">
                    <Textarea
                      label="Description"
                      name={`routine${index + 1}Description`}
                      value={routineSteps[index]?.description}
                    />
                  </div>
                </div>
              </div>

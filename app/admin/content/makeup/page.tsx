...
import { updateMakeupPage } from "./actions";

export default async function AdminMakeupContentPage() {
      ...
      <div className="mx-auto max-w-5xl">
        <div className="border-b border-stone-200 pb-8">
          ...
          <p className="mt-3 max-w-xl text-sm leading-6 text-stone-500">
            Manage the content displayed on your makeup page.
          </p>
        </div>

        <form
          action={updateMakeupPage}
        ...
        >
          ...
          {/* CATEGORIES */}
          <Section title="Makeup Categories">
            <div className="grid gap-6 sm:grid-cols-2">
              <Field
                label="Eyebrow"
          ...
          <Section title="Beauty Looks">
            <div className="grid gap-6 sm:grid-cols-2">
              ...
              <Field
                label="Title Before"
                name="looksTitleBefore"
                value={looks.titleBefore}
              />

              <Field
                label="Highlight"
                name="looksHighlight"
                value={looks.highlight}
              />

              <Field
                label="Button Text"
                name="looksButtonText"
                value={looks.buttonText}
              />
            </div>
            ...
            {Array.from({ length: 3 }, (_, index) => (
              ...
              >
                ...
                <div className="mt-5 grid gap-6 sm:grid-cols-2">
                  <Field
                    label="Title"
                    name={`look${index + 1}Title`}
                    value={lookItems[index]?.title}
                  />

                  <Field
                    label="Filter"
                    name={`look${index + 1}Filter`}
                    value={lookItems[index]?.filter}
                  />

                  <div className="sm:col-span-2">
                    <Textarea
                      label="Description"
                      name={`look${index + 1}Description`}
                      value={lookItems[index]?.description}
                    />
                  </div>
                  ...
                  <div className="sm:col-span-2">
                    <Field
                      label="Image"
                      name={`look${index + 1}Image`}
                      value={lookItems[index]?.image}
                    />
                  </div>
                </div>
              </div>
